...
class _MapLodWidgetState extends State<MapLodWidget> {
  ...
  int _selectedProvinceIndex() {
    ...
    final level = widget.repository.levelForUnit(selectedUnit);
    if (level == MapDetailLevel.communes) {
      final parentMa = selectedUnit.parentMa;
      if (parentMa == null || parentMa.isEmpty) return -1;
      return _provinces.indexWhere((unit) => unit.ma == parentMa);
    }

    final selectedMa = selectedUnit.ma;
    if (selectedMa == null || selectedMa.isEmpty) return -1;
    return _provinces.indexWhere((unit) => unit.ma == selectedMa);
  }

  int _selectedCommuneIndex() {
    if (_detailLevel != MapDetailLevel.communes ||
        _visibleCommunes.isEmpty ||
    ...
    final selectedUnit = widget.selectedUnit;
    if (selectedUnit == null) return -1;
    if (widget.repository.levelForUnit(selectedUnit) !=
    ...
    final selectedMa = selectedUnit.ma;
    if (selectedMa == null || selectedMa.isEmpty) return -1;
    return _visibleCommunes.indexWhere((unit) => unit.ma == selectedMa);
  }
  ...
  MapZoomPanBehavior _createZoomPanBehavior({
  ...
  void _trackCameraFromZoom(MapZoomDetails details) {
    ...
    if (details.focalLatLng != null) {
      _cameraFocal = details.focalLatLng;
    } else if (details.newVisibleBounds != null) {
  ...
  void _refreshCameraFromController() {
    try {
      ...
      if (_zoomPanBehavior.zoomLevel != _zoomLevel) {
  ...
  void initState() {
    ...
    _zoomPanBehavior = _createZoomPanBehavior(
  ...
  MapShapeSource _buildProvinceSource() {
    ...
    if (_provinceSourceCache != null && _provinceSourceCacheKey == cacheKey) {
  ...
  MapShapeSource _buildMemoryShapeSource(List<AdminUnit> data, String geoJson) {
    if (widget.colorMode == ColorMode.byRegion) {
      return _buildRegionMemorySource(data, geoJson);
    }
    return _buildTypeMemorySource(data, geoJson);
  }
  ...
  MapShapeSource _buildTypeMemorySource(List<AdminUnit> data, String geoJson) {
  ...
  Widget build(BuildContext context) {
    ...
    final showCommuneOverlay =
        ...
        _communeGeoJson != null;
    final selectedProvinceIndex = _selectedProvinceIndex();
    final selectedCommuneIndex = _selectedCommuneIndex();
            ...
            Positioned.fill(
              child: SfMapsTheme(
                ...
                child: SfMaps(
                  layers: [
                    MapShapeLayer(
                      key: ValueKey("provinces-${widget.colorMode.name}-${widget.selectedUnit?.ma ?? 'none'}"),
                      ...
                      sublayers: showCommuneOverlay
                          ? [
                              MapShapeSublayer(
                                key: ValueKey("communes-${widget.selectedUnit?.ma ?? 'none'}-${widget.repository.cacheKeyForProvinces(_visibleParentMas)}"),
                                source: _buildCommuneSource(),
                                strokeColor: Colors.white.withValues(
                                  alpha: 0.55,
                                ),
                                strokeWidth: 1.0,
                                selectedIndex: selectedCommuneIndex,
                                selectionSettings: const MapSelectionSettings(
                                  color: Color(0xBB00E5FF),
                                  strokeColor: Color(0xFF00E5FF),
                                  strokeWidth: 3.0,
